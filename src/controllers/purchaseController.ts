...
import { queuePurchaseRequest } from '../queue/producer';


export const purchaseProduct = async (req: Request, res: Response) => {
  const { userId, productId } = req.body;
  console.log("💡 Received purchase attempt:", { userId, productId });
  ...
  try {
    await queuePurchaseRequest({ userId, productId });
    res.status(202).json({ message: "Purchase request queued" });
    return;
  } catch (error) {
